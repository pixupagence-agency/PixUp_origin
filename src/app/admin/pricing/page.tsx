...
export default function AdminPricing() {
    ...
    return (
        <div className="flex h-screen w-full overflow-hidden bg-background-light dark:bg-background-dark transition-colors">
            <AdminSidebar />
            <main className="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark transition-colors">
                <div className="container mx-auto max-w-5xl px-4 py-8 md:px-8">
                    <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{t.admin.pricingManagement}</h1>
                            <p className="mt-1 text-slate-500 dark:text-slate-400">{t.admin.pricingDesc}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {plans.map((plan) => (
                            <div
                                key={plan.id}
                                className={`group rounded-2xl border ${plan.popular ? 'border-2 border-primary shadow-md' : 'border-slate-200 dark:border-slate-800 shadow-sm'} bg-white dark:bg-slate-900 p-6 hover:shadow-lg transition-all relative`}
                            >
                                {plan.popular && (
                                ...
                                <div className="flex flex-col gap-4">
                                    <div className="flex items-start justify-between">
                                        <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${plan.popular ? 'bg-primary text-white' : 'bg-blue-50 dark:bg-blue-900/20 text-primary'}`}>
                                            <span className="material-symbols-outlined text-2xl">{plan.popular ? 'star' : 'eco'}</span>
                                        </div>
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => togglePopular(plan.id)}
                                                className={`rounded-lg p-2 ${plan.popular ? 'text-primary bg-primary/10' : 'text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-primary'} transition-colors`}
                    ...
                    <div className="mt-12 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800">
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t.admin.billings}</h2>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead className="dark:bg-slate-800/50">
                                    <tr className="bg-slate-50/50 dark:bg-transparent">
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t.admin.client}</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t.admin.plan}</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t.admin.amount}</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t.admin.dueDate}</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 text-right">{t.admin.action}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    <tr className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors">
                                        ...
                                        <td className="px-6 py-4"><span className="text-sm text-slate-600 dark:text-slate-400">Professional</span></td>
                                        <td className="px-6 py-4"><span className="text-sm font-bold text-slate-900 dark:text-white">99.00€</span></td>
                                        <td className="px-6 py-4"><span className="text-sm text-slate-500 dark:text-slate-400">March 12, 2026</span></td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border border-transparent dark:border-slate-800">
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">{t.admin.planName}</label>
                                <input
                                    required
                                    type="text"
                                    value={formData.name}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">{t.admin.price}</label>
                                    <input
                                        required
                                        type="text"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">{t.admin.description}</label>
                                <textarea
                                    required
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Features (comma separated)</label>
                                <textarea
                                    required
                            ...
                            <div className="flex items-center gap-6 pt-2">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        ...
                                        className="w-4 h-4 rounded text-primary focus:ring-primary border-slate-300 dark:border-slate-600 dark:bg-slate-700"
                                    />
                                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t.admin.mostPopular}</span>
                                </label>
                            </div>
